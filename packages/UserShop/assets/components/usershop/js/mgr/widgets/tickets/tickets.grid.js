...
UserShop.grid.UserTickets = function (config) {
  config = config || {};
  Ext.applyIf(config, {
    id: "usershop-grid-tickets",
  ...
  UserShop.grid.UserTickets.superclass.constructor.call(this, config);
};

Ext.extend(UserShop.grid.UserTickets, MODx.grid.Grid);

// Контекстное меню
UserShop.grid.UserTickets.prototype.showContextMenu = function (
  record,
  coords
) {
...
// Окно редактирования
UserShop.grid.UserTickets.prototype.updateRecord = function (record) {
    ...
    buttons: [
      {
        text: "Сохранить",
        handler: function () {
          const form = Ext.getCmp("usershop-form-tickets-edit").getForm();
          const values = form.getValues();

          Ext.Ajax.request({
            url: UserShop.config.connector_url,
            params: {
              action: "mgr/tickets/update",
              id: record.get("id"),
            ...
            success: function (response) {
              ...
              if (result.success) {
                win.close();
                Ext.getCmp("usershop-grid-tickets").refresh();
              } else {
...
Ext.reg("usershop-grid-tickets", UserShop.grid.UserTickets);
